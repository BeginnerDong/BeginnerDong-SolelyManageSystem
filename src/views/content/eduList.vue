<template>
  <div class="list">
    <div style="width: 100%;position: relative;margin: 0 auto;height: 15%;overflow: scroll;">
      <div class='field_item' v-for='(field,index) in fields' :index="key"  v-if="field.header_search&&String(field.header_search)!='{}'">
        <component
          :field="field"
        ...
        <template v-slot:expand="expand">
         ...
         <template v-slot:mainImg="mainImg">
          <div style="position: relative;">
            <div style="bottom: 5px;" v-for="(item,index) in mainImg.data.mainImg" :key="index">
              <el-image
              ...
              <div v-else>
                <a :href="item.url" target="view_window">文件名{{item.title}}</a>
              </div>
            </div>
          </div>
        </template>
...
<script>
  import eduListJs from './eduList.js'
  export default eduListJs
</script>
